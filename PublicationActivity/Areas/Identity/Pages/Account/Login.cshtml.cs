    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var existingUser = await this._userManager.FindByEmailAsync(Input.Email);

                if (existingUser == null)
                {
                    ModelState.AddModelError(string.Empty, "Не съществува потребител с посочения имейл.");
                    return Page();
                }

                var result = await _signInManager.PasswordSignInAsync(existingUser, Input.Password, Input.RememberMe, lockoutOnFailure: false);
                if (result.Succeeded)
                {
